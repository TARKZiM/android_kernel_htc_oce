...
static int flash_led0_last = -1;
static int flash_led1_last = -1;
static int torch_led0_last = -1;
static int torch_led1_last = -1;
int pmi8994_flash_mode(int led0_curr, int led1_curr)
{
	FLT_INFO_LOG("%s: camera flash current %d+%d.\n", __func__, led0_curr, led1_curr);

	/* workaround for case:
	   1. during shutdown and backlight off;
	   2. turn off flashlight by camera calling pmi8994_flash_mode(0,0); 
	   3. set_backlight_all(BACKLIGHT_ON) will turn on backlight during shutdown which
	      will lead to current leak of backlight pin after shutdown.
	 */
	if ((led0_curr == flash_led0_last ) && (led1_curr == flash_led1_last)) {
		return 0;
	} else {
		flash_led0_last = led0_curr;
		flash_led1_last = led1_curr;
	}
...
int pmi8994_torch_mode(int led0_curr, int led1_curr)
{
	...
	if ((led0_curr == torch_led0_last ) && (led1_curr == torch_led1_last)) {
		return 0;
	} else {
		torch_led0_last = led0_curr;
		torch_led1_last = led1_curr;
	}
